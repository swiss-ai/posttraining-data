...
benchmarks/RewardBench2-test/overall.html.

Each data column is coloured independently: green = column maximum,
red = column minimum, with a white midpoint in between.

Example (from the 08a-judge-evaluation repo root):

    python -m benchmarks.RewardBench2-test.src.4-compile-results-html
"""

import glob
import json
import os
from typing import Any, Dict, List, Tuple  # Tuple used in col_stats type
...
def _score_to_colour(value: float, col_min: float, col_median: float, col_max: float) -> str:
    """
    Map a score to a background colour, centred on the column median.
    Below median: interpolate red (#e05555) → white (#ffffff)
    Above median: interpolate white (#ffffff) → green (#4caf73)
    """
    if value <= col_median:
        denom = col_median - col_min
        s = (value - col_min) / denom if denom > 0 else 1.0
        # red (224,85,85) → white (255,255,255)
        r = int(224 + (255 - 224) * s)
        g = int(85 + (255 - 85) * s)
        b = int(85 + (255 - 85) * s)
    else:
        ...
        # white (255,255,255) → green (76,175,115)
        r = int(255 + (76 - 255) * s)
        g = int(255 + (175 - 255) * s)
        b = int(255 + (115 - 255) * s)
...
def _build_html(
    ...
    # Per-column min/median/max for colour scaling
    ...
    for _, row in df_scores.iterrows():
        ...
        for col in data_cols:
            ...
            if none_row is not None and col in none_row.index and not pd.isna(none_row[col]):
                none_rate = float(none_row[col])

            text = f"{score_f:.2f}"
            is_best = abs(score_f - col_max) <= 1e-9
            sup_html = f"<sup>{none_rate:.0f}%</sup>" if none_rate > 0 else ""
            cell_class = "td num best" if is_best else "td num"
            cells.append(
...
<head>
<meta charset="utf-8">
<title>RewardBench-2 results</title>
<style>{css}</style>
</head>
<body>
<h3>RewardBench-2 — accuracy (higher is better)</h3>
...
<p style="font-size:11px;color:#000">
  Superscript = % Prompts with ≥1 None-scored response.  <strong>Bold</strong> = column best.
</p>
<table>
...
if __name__ == "__main__":
    ...
    out_path = "benchmarks/RewardBench2-test/overall.html"
