...
3) Ensures ALL messages are minimal dicts with ONLY: {"role", "content"}
...
def minimal(msg: Dict[str, Any]) -> Dict[str, str]:
    """Reduce a message to {'role','content'} only (strip whitespace)."""
    return {"role": msg.get("role"), "content": (msg.get("content") or "").strip()}

def promote_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    """Insert thought messages before cleaned assistant; drop all other keys."""
    out: List[Dict[str, str]] = []
    for m in messages or []:
        role = m.get("role")
        if role != "assistant":
            out.append(minimal(m))
            continue

        cleaned, thinks = extract_thinks(m.get("content", ""))
        for t in thinks:
...
def process_batch(batch: Dict[str, List]) -> Dict[str, List]:
    ...
    for i in range(n):
        ...
        for br in branches:
            ...
            br["messages"] = promote_messages(msgs)
