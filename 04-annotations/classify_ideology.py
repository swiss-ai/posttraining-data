...
on a scale of 0-3, where 0 indicates no ideological sensitivity and 3 indicates
high sensitivity where ideological position would fundamentally shape responses.
"""
...
def collect_classification_tasks(sample: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
...
async def classify_dataset(input_path: Path, output_path: Path, model: str, 
    ...
    """
    Classify ideological sensitivity in a chat format dataset.
    ...
    prompt_template_path = Path(__file__).parent / "prompts" / "ideological.txt"
    ...
    for chunk_idx, chunk_start in enumerate(range(0, len(dataset), chunk_size)):
        ...
        for i, sample in enumerate(tqdm(chunk, desc="Collecting tasks")):
            try:
                task = collect_classification_tasks(sample)
                if task:
        ...
        if not chunk_tasks:
            print("No initial prompts found in this chunk")
            # Add samples without modifications
            all_updated_samples.extend(chunk)
            continue
        
        print(f"Found {len(chunk_tasks)} samples to classify")
        
        # Format tasks for classification
        formatted_tasks = []
        for task in chunk_tasks:
            # Format as required by classify_single (using "question" and "answer" keys)
            formatted_tasks.append({
                "question": task["system_prompt"],  # Will be used for {system_prompt}
                "answer": task["initial_prompt"]    # Will be used for {initial_prompt}
            })
        ...
        # Add any samples that weren't classified (no initial prompt)
    ...
    processing_entry = {
        "operation": "ideological_classification",
        "script": "classify_ideology.py",
        "timestamp": datetime.now().isoformat(),
        ...
        "samples_classified": total_classifications,
    ...
    print(f"\n=== IDEOLOGICAL CLASSIFICATION COMPLETED ===")
    ...
    print(f"Samples classified: {total_classifications:,}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Classify ideological sensitivity in chat format datasets",
...
Examples:
  ...
  python classify_ideology.py data/02-standardised/smoltalk \\
    ...
    print("=== Ideological Classification Configuration ===")
