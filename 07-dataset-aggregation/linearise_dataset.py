...
def linearise_sample_for_sft(sample: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if "available_functions" in sample:
        ...
        for tool in sample["available_functions"]:
            ...
            if "properties" in tool_parameters and isinstance(
                tool_parameters["properties"], str
            ):
                ...
                # For now we assume the tool calls are done sequentially
                    ...
                    if isinstance(tool_arguments, dict):
                        try:
                            ...
                            print(
                                f"Warning: Failed to parse tool arguments: {tool_arguments}"
                            )
                            tool_arguments = "{}"
