...
# --- input adapters ------------------------------------------------------
# Patch B: read raw parquet shards directly so we never materialize a
# standardised HF DatasetDict on disk. Each adapter returns parallel lists
# of (prompt_texts, conversation_ids).

_IMG_TOK = re.compile(r"<image>\n?")


def _load_parquet_sharegpt_vision(path: str) -> Tuple[List[str], List[str]]:
    """ShareGPT-style vision SFT: rows have `id` and `conversations` with
    list of {from, value}. Prompt = first user turn, image tokens stripped.

    Streams via iter_batches() to avoid pyarrow's chunked-nested read bug on
    multi-row-group parquet files."""
    ...
    for i, f in enumerate(files, 1):
        pf = pq.ParquetFile(f)
        for batch in pf.iter_batches(columns=["id", "conversations"], batch_size=10000):
            ids = batch.column("id").to_pylist()
            convs_list = batch.column("conversations").to_pylist()
            for rid, convs in zip(ids, convs_list):
                if not convs:
                    continue
                prompt = _IMG_TOK.sub("", convs[0]["value"]).strip()
                if not prompt:
                    continue
                prompts.append(prompt)
                conv_ids.append(rid)
            if len(prompts) - last_print >= 200000:
                print(f"  loaded {len(prompts)} prompts so far (shard {i}/{len(files)})")
                last_print = len(prompts)
        if i % 50 == 0 or i == len(files):
            print(f"  finished shard {i}/{len(files)}, {len(prompts)} prompts so far")
    return prompts, conv_ids


PARQUET_ADAPTERS = {
    "sharegpt-vision": _load_parquet_sharegpt_vision,
}
...
def main(args):
    ...
    # Patch: drop benchmarks matching any substring in --exclude-benchmarks
    # (case-insensitive — there are duplicate sets like
    # 'swiss-ai__polyglotoxicityprompts__*' and
    # 'ToxicityPrompts__PolygloToxicityPrompts__*').
    if args.exclude_benchmarks:
        patterns = [p.strip().lower() for p in args.exclude_benchmarks.split(",") if p.strip()]
        before = len(benchmark_list)
        benchmark_list = [b for b in benchmark_list if not any(p in b.lower() for p in patterns)]
        print(f"Excluded {before - len(benchmark_list)} benchmarks matching: {patterns}")
    ...
    # Patch B: parquet-direct input avoids materializing an HF DatasetDict
    # on disk. Patch A path keeps the DatasetDict route but maps in-memory.
    if args.input_format in PARQUET_ADAPTERS:
        print(f"Loading parquet input via adapter '{args.input_format}' from {args.dataset_path}")
        prompts, train_conversation_ids = PARQUET_ADAPTERS[args.input_format](args.dataset_path)
        print(f"Tokenizing {len(prompts)} training prompts (no on-disk cache)...")
        # Tokenize in batches, keep token_ids list of lists in RAM.
        train_data_prompts_token_ids: List[List[int]] = []
        BATCH = 1000
        for i in range(0, len(prompts), BATCH):
            train_data_prompts_token_ids.extend(
                tokenizer(prompts[i:i + BATCH])["input_ids"]
            )
        del prompts
    else:
        # Standardised HF DatasetDict path (Patch A: in-memory map).
        train_data = load_from_disk(args.dataset_path)
        if hasattr(train_data, 'keys'):  # DatasetDict
            all_splits_data = [train_data[k] for k in train_data.keys()]
        ...
        print("Tokenizing training prompts (in-memory)...")
    ...
    # Patch A: no filtered DatasetDict is written. We just merge the per-
    # benchmark reports into a single summary.json keyed by training id.
    print(f"\n=== FINAL SUMMARY ===\nCombining contamination reports from {len(processed_benchmarks)} processed benchmarks...")
    del eval_data
    gc.collect()
    contaminated_ids: set = set()
    per_benchmark: List[Tuple[str, int]] = []
    id_to_benchmarks: Dict[str, List[str]] = defaultdict(list)
    for eval_dataset_name in processed_benchmarks:
        ...
        with open(report_path, "r") as f:
            report = json.load(f)
        if len(report) > 0:
            per_benchmark.append((eval_dataset_name, len(report)))
            for cid in report:
                id_to_benchmarks[cid].append(eval_dataset_name)
            contaminated_ids.update(report.keys())

    n_train = len(train_conversation_ids)
    print(f"\n=== CONTAMINATION SUMMARY ===")
    print(f"Total training samples scanned: {n_train}")
    rate = (len(contaminated_ids) / n_train) if n_train else 0.0
    print(f"Total contaminated samples    : {len(contaminated_ids)} ({rate * 100:.4f}%)")
    if per_benchmark:
        per_benchmark.sort(key=lambda x: x[1], reverse=True)
        print(f"\nTop benchmarks with contamination:")
        for benchmark, count in per_benchmark[:30]:
            print(f"  {count:6d}  {benchmark}")
        if len(per_benchmark) > 30:
            print(f"  ... and {len(per_benchmark) - 30} more benchmarks")

    summary_path = os.path.join(args.report_path, "summary.json")
    summary = {
        ...
        "per_benchmark_counts": dict(per_benchmark),
        "contaminated_ids": sorted(contaminated_ids),
        "id_to_benchmarks": dict(id_to_benchmarks),
        "timestamp": datetime.now().isoformat(),
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "dataset_path",
        type=str,
        help="Path to the training dataset. Either an HF DatasetDict directory (standardised format) "
             "or a parquet file/directory when --input-format selects a parquet adapter.",
    )
    parser.add_argument(
        "--input-format",
        type=str,
        default="standardised",
        choices=["standardised", *PARQUET_ADAPTERS.keys()],
        help="Input loader. 'standardised' = HF DatasetDict via load_from_disk. "
             "Other values pick a parquet adapter (avoids writing a DatasetDict copy).",
    )
    parser.add_argument(
        "--exclude-benchmarks",
        type=str,
        default=None,
        help="Comma-separated substrings; any benchmark whose name contains one is skipped.",
    )
    parser.add_argument(
